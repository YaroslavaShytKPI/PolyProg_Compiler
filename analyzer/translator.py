...
class Translator:
    ...
    def parse_id(self):
        ...
        if tok in "id":
            ...
            ####################
            self.postfix_code.append((lex, tok))
            if self.to_view:
                self.configToPrint(lex, self.num_row)
            # там у нього є ще інший код, але я хз чи він потрібен
            ####################
    ...
    def parse_assign(self):
        ...
        #######################
        self.postfix_code.append(("=", "assign_op"))
        if self.to_view:
            self.configToPrint('=', self.num_row)
        #######################
    ...
    def parse_expression(self):
        ...
        while F:
            ...
            if tok in ("add_op", "power_op", "mult_op"):
                ...
                #########################
                if(lex == '+'):
                    self.postfix_code.append(('POS', tok))
                    if self.to_view: 
                        self.configToPrint('POS', self.num_row)
                elif(lex == '-'):
                    self.postfix_code.append(('NEG', tok))
                    if self.to_view: 
    ...
    def parse_term(self):
        ...
        while F:
            ...
            if tok in "mult_op":
                ...
                #############################
                for row in temp_pow_op_holder:
                    self.postfix_code.append(row[0], row[1])
                    if self.to_view:
                ...
                exp_type = self.get_op_type(parsed_factor_token_left, lex, parsed_factor_token_right)
            # не маю поняття чи це правильно просто роблю як у прикладі
            #################################
            #elif tok == 'pow_op':
            #   temp_pow_op_holder.append((lex, tok, self.num_row))
            #
            #################################
    ...
    def parse_factor(self):
        ...
        # можливо тут треба якось обробляти, поки не зроз
    ...
    def parse_for(self):
        ...
        if lex == "for" and tok == "keyword":
            ...
            ################################
            self.postfix_code.append(leave)
            self.postfix_code.append(('JF', 'jump'))
            self.postfix_code.append(action)
            self.postfix_code.append(('JMP', 'jump'))
            ...
            ################################
            self.postfix_code.append(start)
            self.postfix_code.append(('JMP', 'jump'))
            self.setValLabel(action)
            ###############################
            ...
            ###############################
            self.postfix_code.append(increment)
            self.postfix_code.append(('JMP', 'jump'))
    ...
    def createLabel(self):
        ...
        lexeme = "m"+str(nmb)
        self.tableOfLabel[lexeme] = 'value_undef'
        tok = 'label'
        return (lexeme, tok)
    
    def setValLabel(self, lbl):
        lex, _tok = lbl
        self.tableOfLabel[lex] = len(self.postfix_code)
        return True
    ...
    def configToPrint(self, lex, numRow):
        ...
        print(stage.format(lex, numRow, str(
            table_of_sym[self.num_row]), str(self.postfix_code)))
...
translator = Translator().postfixTranslator()
